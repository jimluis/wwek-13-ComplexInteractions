...
public class PushButton : MonoBehaviour
{
    ...
    [Min(0.0001f)]
    public float pressThreashold = 0.001f;

    [Min(0.0001f)]
    public float resetThreashold = 0.001f;

    [Min(0.01f)]
    public float returnSpeed = 1.0f;

    float m_currentPressDepth = 0f;
    float m_yMax = 0.05f;//resting position
    float m_yMin = 0.05f;//all the way pressed in
    ...
    bool IsPressed()
    {
        return transform.localPosition.y >= m_yMin && transform.localPosition.y <= m_yMin + pressThreashold;
    }

    bool IsReset()
    {
        return transform.localPosition.y >= m_yMax - resetThreashold && transform.localPosition.y <= m_yMax;
    }
